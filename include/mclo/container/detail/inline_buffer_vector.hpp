...
namespace mclo::detail
{
	...
	class inline_buffer_vector_header
	{
	public:
		...
		inline_buffer_vector_header( std::byte* const data, size_type capacity ) noexcept
			: m_data( data )
			, m_capacity( capacity )
		{
		}
	...
	/// @brief Size helper mimics a inline_buffer_vector size 1 to find the variable offset
	template <typename T>
	struct inline_buffer_vector_offset_helper
	{
		inline_buffer_vector_header m_header;
	...
	/// @brief inline_buffer_vector size independent code, mimics the API of std::vector
	/// @details Can not be constructed directly should only be used as a reference type in functions
	/// which should operate on any sized inline_buffer_vector of a given type.
	/// @tparam T Type of objects stored
	/// @tparam CanGrow If the vector can grow outside its inline capacity
	template <typename T, bool CanGrow>
	class inline_buffer_vector_base : private detail::inline_buffer_vector_header
	{
		using base = detail::inline_buffer_vector_header;
	...
	public:
		...
		inline_buffer_vector_base( const inline_buffer_vector_base& other ) = delete;
		inline_buffer_vector_base( inline_buffer_vector_base&& other ) noexcept = delete;

		inline_buffer_vector_base& operator=( const inline_buffer_vector_base& other )
		{
			if ( this != &other )
			{
		...
		inline_buffer_vector_base& operator=( inline_buffer_vector_base&& other ) noexcept
		{
			if ( this == &other )
			{
				return *this;
		...
		inline_buffer_vector_base& operator=( std::initializer_list<value_type> init_list )
		{
			assign( init_list );
			return *this;
		}

		void swap( inline_buffer_vector_base& other )
		{
			if ( this == &other )
			{
				return;
		...
		friend void swap( inline_buffer_vector_base& lhs, inline_buffer_vector_base& rhs )
		{
			lhs.swap( rhs );
		}
		...
		iterator erase( const_iterator pos ) MCLO_NOEXCEPT_TESTS_IF( std::is_nothrow_move_assignable_v<value_type> )
		{
			DEBUG_ASSERT( pos >= begin() && pos <= end(), "pos must be an iterator in this container" );
	...
	private:
		...
		public:
			...
			void set_begin( const pointer new_first ) MCLO_NOEXCEPT_TESTS
			{
		...
		[[nodiscard]] static uninitialized_unique_ptr allocate_uninitialized( const size_type amount )
		{
			...
			{
				return uninitialized_unique_ptr{
					static_cast<pointer>( ::operator new( sizeof( value_type ) * amount ) ) };
			}
			else
			{
				PANIC( "Attempting to grow fixed capacity inline buffer vector", amount );
			}
		}
		...
		void swap_with_larger( inline_buffer_vector_base& larger )
		{
		...
		static constexpr std::size_t offset = offsetof( detail::inline_buffer_vector_offset_helper<T>, m_data );
	...
	constexpr std::size_t default_inline_capacity_bytes =
		std::hardware_destructive_interference_size - sizeof( inline_buffer_vector_header );

	template <typename T>
	constexpr std::uint32_t default_inline_capacity = [] {
		static_assert( sizeof( T ) <= 256,
					   "You are trying to use a default number of inlined elements for "
					   "`inline_buffer_vector<T>` but `sizeof(T)` is really big! Please use an "
					   "explicit number of inlined elements with `inline_buffer_vector<T, N>` to make "
					   "sure you really want that much inline storage." );
		// Give capacity for at least one element as long as static assert passes
		// Allows a inline_buffer_vector<inline_buffer_vector<T>> to have at least one element which is
	...
	/// @brief inline_buffer_vector stores Capacity elements inline on the stack but can grow and allocate on the heap
	/// additional, mirrors std::vector API
	/// @details inline_buffer_vector_base<T> provides a size erased type that provides all the same functions for code
	/// generic over size
	/// @tparam T Type of objects stored
	/// @tparam CanGrow If the vector can grow outside its inline capacity
	...
	template <typename T, bool CanGrow, std::uint32_t Capacity = detail::default_inline_capacity<T>>
	class inline_buffer_vector : public inline_buffer_vector_base<T, CanGrow>
	{
		using base = inline_buffer_vector_base<T, CanGrow>;
	...
	public:
		...
		inline_buffer_vector( inline_buffer_vector&& other ) noexcept
			: inline_buffer_vector()
		{
			base::operator=( std::move( other ) );
		}

		inline_buffer_vector& operator=( const inline_buffer_vector& other )
		{
			base::operator=( other );
			return *this;
		}

		inline_buffer_vector& operator=( inline_buffer_vector&& other ) noexcept
		{
			base::operator=( std::move( other ) );
			return *this;
		}

		explicit inline_buffer_vector( const size_type count )
			: inline_buffer_vector()
		{
			base::resize( count );
		}

		inline_buffer_vector( const size_type count, const_reference value )
			: inline_buffer_vector()
		{
		...
		template <std::input_iterator It>
		inline_buffer_vector( It first, It last )
			: inline_buffer_vector()
		{
			base::assign( first, last );
		}

		inline_buffer_vector( std::initializer_list<value_type> init_list )
			: inline_buffer_vector()
		{
