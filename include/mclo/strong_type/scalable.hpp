...
namespace mclo::strong_type
{
	...
	/// @details Models scaling a strong value by a plain scalar, e.g. mclo::strong_type::type<double, struct tag,
	/// mclo::strong_type::scalable_with<double>> so that meters * 2.0 yields meters, without permitting the
	/// dimensionally meaningless strong * strong product that multipliable would allow. Multiplication is provided with
	/// the scalar on either side; division is only provided as Derived / Scalar. Compound assignment forms (operator*=
	/// and operator/=) are also generated.
